...
from .se_module import SELayer, ECANet
import pdb
...
class ShuffleV2Block(nn.Module):
    def __init__(self, inp, oup, stride, activation="ReLU", with_se=False, with_eca=True):
        ...
        if self.stride > 1:
            ...
            if self.with_se:
                self.branch2_2 = SELayer(branch_features)
            if self.with_eca:
                self.branch2_2 = ECANet(branch_features)
            ...
            if self.with_se:
                self.branch2_2 = SELayer(oup)
            if self.with_eca:
                self.branch2_2 = ECANet(oup)
            self.pw = nn.Sequential(
                nn.Conv2d(
                    oup,
                    branch_features,
    ...
    def forward(self, x):
        ...
        if self.stride == 1:
            ...
            tmp = torch.cat((x2_1, x2_), dim=1)
            if self.with_se:
                tmp = self.branch2_2(tmp)
            if self.with_eca:
                tmp = self.branch2_2(tmp)
            x2_3 = self.pw(tmp)
            out = torch.cat((x2_3, x1), dim=1)
            out = channel_shuffle(out, 2)

        else:
            # pdb.set_trace()
            x1 = self.branch1(x)
            tmp = self.branch2_1(x)
            if self.with_se:
                tmp = self.branch2_2(tmp)
            if self.with_eca:
                tmp = self.branch2_2(tmp)
            x2_3 = self.branch2_3(tmp)
            out = torch.cat((x2_3, x1), dim=1)
            out = self.dw(out)
            out = self.pw(out)
...
class EsNet(nn.Module):
    def __init__(
        ...
        if model_size == "0.5x":
            ...
            self._stage_out_channels = [24, 96, 192, 384, 1024]
        ...
        for name, repeats, output_channels in zip(
            ...
            seq = [
                ShuffleV2Block(
                    input_channels, output_channels, 2, activation=activation, with_se=with_se, with_eca=with_eca
                )
            ]
            for i in range(repeats - 1):
                seq.append(
                    ShuffleV2Block(
                        output_channels, output_channels, 1, activation=activation, with_se=with_se, with_eca=with_eca
                    )
                )
